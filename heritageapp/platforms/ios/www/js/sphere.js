		...
		console.log("sphere_image_list" + sphere_image_list);
			if(sphere_image_list == null){
					...
					var query = AzureTable.where({}).read().done(function (results) {
						...
						init();
						animate();
						localStorage.sphere_image_list = JSON.stringify(sphere_image_list);
						}, function (err) {
						if(!(typeof localStorage.sphere_image_list === 'undefined'))
    						sphere_image_list = JSON.parse(localStorage.sphere_image_list);
		...
		//console.log("sphere_image_list" + sphere_image_list);
			if(sphere_image_list == null){
					...
					var query = AzureTable.where({}).read().done(function (results) {
						...
						//console.log("url " + sphere_image_list[0].part_top);
						runanimation = true;
						image_index360 = get_tour_sphere();
						init();
						animate();
						}, function (err) {
						alert("Error: " + err + " Check your internet connection.");
...
function get_tour_sphere()
{
	for(var i =0; i<sphere_image_list.length; i++)
	{
		console.log(sphere_image_list[i].id);
		if(sphere_image_list[i].id === sphere_to_load)
			{
				return i;
			}
	}
