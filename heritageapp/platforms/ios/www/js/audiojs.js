...
function getTourList1()
{
	...
	if(tour_list == null)
	{
		...
		language:lang_index
		}).take(1000).read().done(function (results) {
			tour_list = results;
			var panel_array;
			console.log("IN GET LIST2");
			for(var i = 0 ; i < tour_list.length;i++)
			{
				//console.log(tour_list[i].panel_numb+" "+tour_list[i].title+" "+tour_list[i].panel_type+" "+tour_list[i].lat+" "+tour_list[i].lon+" "+tour_list[i].quest_type);
...
{	
	...
	for(var i = 1; i < tourDict.length;i++)
	{
		try 
		{
			console.log(tourDict[i][0].title);
			...
			for(var j = 1; j < tourDict[i].length;j++)
			{
				if(tourDict[i][j].panel_type == "child" && tourDict[i][j].quest_type == "info")
				{
					...
					$(child_div).append("<div id = \""+j+" \"class=\" sound_tour child_audio\"><div id=\"audio_button\" class=\"audio_red row_audio\" style=\"\" onclick=\"\"></div>"+  tourDict[i][j].title +"</div>");
				}
			}
			if(children == true)
			{
...
{
	var childrenPanelsLen;
	for(var i = 1; i < tourDict.length;i++)
	{
		try
		{
			//a check to see if tourDict[i] exists or not
			childrenPanelsLen =  tourDict[i].length;
		}
		catch(err)
		{
			continue;
