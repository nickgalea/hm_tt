...
function getTourList()
{
	if(tour_list == null)
	{
		...
		query = query.take(1000).read().done(function (results) 
		{
			...
			$("#loading").remove();
			console.log("swagatron" + tourDict.length);
			console.log(tourDict[38])
		}, function (err) {
...
function getAllTourImages()
{
	...
	var query = resTable.where({
		...
		console.log("Tour dictionary length   " + tourDict.length);
		fillData_Tour();
	}, function (err) {
		alert("Error: " + err);
	}); 
...
function replace_imagetags()
{
	...
	for(var i = 0; i<total_images; i++)
	{
		...
		if(img_type === "image")
		{
			...
			description = description.replace("<<"+key+">>", "<div class=\"media tour-media\"><img width='100%' src="+imgurl+" alt='img'></div>");
		}
...
function getSelectedArtefact(current_child_num)
{
	...
	{
		if(artefact_list[i].reference === info)
		{
			artefact_position = i;
			break;
		}
